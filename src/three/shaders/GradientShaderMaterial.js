...
float cnoise(vec3 P){
  ...
  return 2.2 * n_xyz;
    ...
    uniform float uTime, uOffset, uAmplifier;

    varying vec2 vUv;
    varying vec3 vNormal, vPosition;

    void main(){
        float noise = cnoise(position + uTime);
...
float cnoise(vec3 P){
  ...
  return 2.2 * n_xyz;
    ...
    float lines(vec2 uv, float offset, float amplifier){
        return smoothstep(
            0.0,0.5 + offset * 0.5, 
            abs(0.5 * sin(uv.x * amplifier) + offset * 2.0)
        );
    }
    ...
    void main()
    {
        float noise = cnoise(vPosition * uAmplifier + uTime);
